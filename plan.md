...
根 package 在整个 MVP 中保持空。下述跨 package `pub`/opaque type 只是 MoonBit package 间编译边界；产品不发布或承诺 library API。每个里程碑仍运行 `moon info`、提交并审查生成 `.mbti`，目的是发现意外边界变化，不是建立兼容性承诺。
...
本节的 M 编号是 capability gate，不与 PR 一一对应；第 10 节的 PR-00 至 PR-12 是唯一 merge 顺序。对应关系固定为：M0=PR-00，M1=PR-01..02，M2=PR-03，M3=PR-04..05，M4=PR-06，M5=PR-07，M6=PR-08，M7..M8=PR-09，M9=PR-10，M10=PR-11，M11=PR-12。canonical encoder 从 PR-01 起随 IR 增量实现。
...
- 将整个仓库的目标许可固定为 Why3 的 LGPL 2.1，并原样附带 Why3 special linking exception。PR-00 同时更新根 `LICENSE`、`moon.mod` 的 license 字段及 README/NOTICE；任何逐段或逐行翻译保留对应 Why3 copyright，并记录翻译/修改日期。此项不再留作后续法律决策。
...
- PR corpus 固定为 PR-00 提交的 `tools/contracts/pr-corpus-v1.json` 中完整、有序、逐 fixture/goal 枚举的 inventory，并锁定其 SHA-256。该文件不得使用目录 glob、数量范围或“之后再选”的占位；增删 entry 必须走显式 schema/profile 变更。
...
- M0/PR-00 完成前，不得开始会固化 identity、snapshot 或 canonical schema 的实现 PR；native runner spike 不接触这些 schema，可以独立并行。
...
PR-00 的正式 lock 文件不得含占位值：除以上固定值外，还要记录 `z3_487.drv` 的完整递归 driver import closure、全部 Theory/Pmodule/use/clone closure inventory 与 hash、feature/corpus/schema manifest hash、OCI image digest、action full commit、Why3/Z3 executable hash。MoonBit 实际版本仅作为 CI 诊断，不进入 lock。PR-04 生成 candidate tables 时在 snapshot manifest 中新增实际 stdlib/driver exporter hash；PR-07 增加 trace patch SHA-256、目标 Why3 commit 和 checkpoint 序列。这些后续字段只能填入已冻结 schema 的对应 artifact manifest，不能反向改写 PR-00 的 identity/schema 决策；任一已存在字段漂移都使 oracle 失败，而不是自动更新 golden。
...
PR corpus 将每条记录的完整 canonical JSON、完整 SMT normalized token stream 及各自 hash checked in；不能只提交 hash。nightly 全量内容不进入 git，只保存为 CI artifact，summary 中保留 hash 与 diff 索引。
...
PR corpus 不再用数量范围描述。唯一权威清单是 PR-00 checked-in 的 `tools/contracts/pr-corpus-v1.json`：它逐项枚举 fixture path、source hash、unit、goal ordinal/name bytes、feature tags、expected stage/kind/lane，并以 manifest SHA-256 冻结。下列每一类必须由该清单中的具名 entry 覆盖；PR-00 未填满并核对清单前不得开始依赖 corpus 的实现 PR：
...
当前 Dockerfile 只安装 Why3，没有安装 Z3。PR-00 先冻结 `linux/amd64` oracle image contract；加入强 gate 前必须：
...
- PR oracle job 的 image reference 直接写入与 `toolchain-lock.json` 相同的 64-hex immutable digest，不使用变量占位或可移动 tag；所有 action 直接使用 lock 中的 full commit SHA；
...
```text
PR-00  license + feature/schema/oracle/toolchain locks + full driver/theory inventory
PR-01  identity + OriginKind + canonical base encoding + all-target SHA-256
PR-02  Ty/Dty + polymorphic unification/generalization + trusted Range/Float schema/canonical
PR-03  Symbol/Term/Decl + string constants + trusted Ddata schema/canonical
PR-04  OCaml snapshot exporter + recursive driver-closure exporter + candidate generated tables
PR-05  Theory/Task + minimal MLW shell + FrozenEnvironment + snapshot loader/catalog + raw snapshot gate
PR-06  pure-logic elaborator + user polymorphism + OCaml typed/raw differential harness
PR-07  complete static Z3 profile + instrumented per-checkpoint transforms including polymorphic path
PR-08  SMT-LIB printer + generated-name normalizer + token differential
PR-09  result parser + native runner + CLI skeleton + pure-logic closure A
PR-10  monomorphic program IR + Real routines/expressions + program elaborator
PR-11  Kode/classical WP + VC differential
PR-12  closure B + complete CLI + CI/golden hardening
```

可以并行：

- native runner spike 可独立并行；spike 不提交 identity/schema/profile 决策，也不改变正式 runner/CLI error protocol。其他 PR 按上表依赖顺序推进，只有在其上游 schema 已合并后才可拆分不共享文件的实现工作。

不能提前：

- PR-00 未完成时，PR-01 及任何会固化 identity/snapshot/canonical schema 的 PR 不得开始；
...
canonicalizer 从 PR-01 起随 IR 增量实现并为每个新增 variant 添加 cross-language vector；不得推迟到 raw Task differential PR。每个 PR 对受影响 target 执行对应 `moon check/test`；PR-12 最后执行 `moon info`、`moon fmt` 并要求再次运行后无 diff。
...
- [ ] feature manifest 穷尽 Ptree variants/形态/拒绝阶段/error kind/fixture/lane；PR corpus 是具名、hash 锁定的完整 inventory。
...
- [ ] 原版 oracle 分层比较 typed semantic/full canonical raw/checkpoints/prepared/SMT/result；PR 提交完整内容，nightly 内容只作 artifact。
