...
> Why3 1.7.2 源码: ../why3
>
> 首个 Z3 兼容 profile：`z3_487.drv`，CI 参考版本先固定为 Z3 4.8.12

本计划描述从现有 WhyML Parser 开始，直到能够对一个严格限定但实用的 WhyML 子集生成 VC、打印 SMT-LIB 2.6、调用 Z3 并返回兼容 Why3 的结果。计划不要求实现 Why3 的其他前端、插件机制或通用 `.drv` 解释器。
...
| 层 | 最小功能 |
|---|---|
| 输入 | 单个 `.mlw` 文件；`theory` 和 `module` |
| 基础环境 | `BuiltIn`、`Bool`、`Unit`、`int.Int`；`real.Real` 在纯逻辑闭环内一并支持 |
| 名字空间 | 当前文件内的 theory/module；`use`、`use import`；精确解析内建最小标准库 |
| 类型 | `bool`、`int`、`real`、`unit`、单态无解释类型、类型变量的内部表示 |
| 纯逻辑声明 | 抽象/别名 `type`、`constant`、非递归 `function`、`predicate`、`axiom`、`lemma`、`goal` |
| 纯逻辑 term | 变量、精确数值、应用、等号、布尔联结、`let`、`if`、`forall`、`exists`、trigger、属性和显式 cast 的语义处理 |
| 程序声明 | 非递归、无副作用的顶层 `let` 函数；抽象 `val` 可作为被调用 routine |
| 程序表达式 | 变量、常量、纯逻辑应用、routine 调用、`let`、`if`、`assert`、`assume` |
| 合约 | `requires`、普通 `ensures`、简单 result pattern；调用者使用被调用者 pre/post |
| VC | classical WP；函数前置/后置条件；分支、绑定、调用、assert/assume |
| SMT | SMT-LIB 2.6；Bool/Int/Real、无解释 sort/function/predicate、量词和 trigger |
| Prover | 静态 Z3 profile；超时；有界 stdout/stderr；稳定结果分类 |
| 验证 | 与 Why3 1.7.2 分层差分，而不只比较最终 solver 答案 |

### 2.2 必须显式拒绝

以下语法可以继续被 Parser 接受，但语义层必须返回带源码位置的 `UnsupportedFeature`，不能静默近似：

- `clone` 和 module interface 的完整语义；
- algebraic datatype、record、constructor、pattern matching；
- inductive/coinductive；
- lambda、epsilon 和高阶应用；
- 残余多态任务及其 SMT encoding；
- 递归函数和 `variant`；
- `while`、`for`、循环不变式；
...
- `vc:sp`、`vc:wp` 等改变 Kode/SP 路径的程序级 VC 控制属性；
- range、float、bitvector、string theory；
...
- 不要求内部对象布局与 OCaml 相同，但 typed semantic、Task、SMT 和结果必须可分层比较。
...
- 根 package 暂无公开 API，适合继续保持为空，直到语义 API 稳定。
...
core/identity/
  ...
  context.mbt                 # opaque context token 与 local ID 分配器
  *_test.mbt
...
core/logic/
  ...
  term_subst.mbt
  term_freevars.mbt
  decl.mbt                    # Dtype/Dparam/Dlogic/Dprop + trusted Ddata view
...
stdlib/
  snapshot_manifest.mbt       # Why3 1.7.2 Theory+Pmodule semantic closure/hash
  handles.mbt                 # 同 context 的 StdlibHandles key->semantic index
...
tools/
  ...
  why3_oracle/                # 测试专用 OCaml oracle
    export_stdlib.ml           # 导出 clone-expanded semantic closure
...
`pipeline.check` 每次新建一个 `CompilationContext`，调用 `minimal_env`，并在 opaque `CheckedFile` 内共同持有 `Environment + StdlibHandles + typed units + raw goals`；`prepare_proof` 只能用其中的 handles 绑定 `Z3StaticSpec`，从 API 形状上阻止 caller 混入另一 context 的 profile。
...
- `CompilationContext` 持有不可伪造、不可序列化的 context token 和 Int64 递增 local ID；`SemanticId` 逻辑上是 `(context token, local ID)`，不能只比较 local ID。
...
- user name、generated name、source span、attributes 分字段保存。
...
- 延迟类型 `Dty` 只存在于 elaborator，最终 semantic `Ty` 不含未解统一变量。
- 统一必须包括 path compression、occurs check、arity check 和 strict finalize。
- 未解类型变量在 strict finalize 时是类型错误，不得留到 printer 猜测。
...
- canonicalizer 为 bound variable 使用 de Bruijn index，为生成符号使用首次出现序号。
...
`UnsupportedFeature` 至少包含 feature enum、stage、span 和简短说明。用户输入错误与内部 invariant failure 必须区分。
...
### M0：冻结契约、oracle 和许可策略

#### 实现项

- 固定 Why3 `1.7.2`、fixture commit、shape version、driver closure、prover detection data 和 semantic stdlib snapshot。
- 新建 MVP feature manifest，lane 为：
...
- 建立 `tools/why3_oracle` 和 NDJSON schema 的空骨架。
- 在 CI 开始时断言 Why3、Moon、Z3 精确版本。
- 决定源码迁移的许可策略：Why3 源码为 LGPL 2.1 加特殊例外；若逐行翻译算法/代码，先确认项目许可与 attribution。行为差分和 clean-room 重实现不等于许可结论。
...
正式 lock 文件要记录 `z3_487.drv` 的递归 import closure，而不只记录以上文件，还要记录 `stdlib/snapshot_manifest.mbt` 所列的 Theory/Pmodule/closure hash 和 stdlib 导出器 hash。M5 创建测试专用的 Why3 driver checkpoint patch 后，还必须把 patch 自身的 SHA-256 和被 patch 的 Why3 commit 写入 lock；任何一项漂移都使 oracle 失败，而不是自动更新 golden。
...
- 每个新 MVP fixture 都有明确 feature tags 和 lane。
...
- `CompilationContext`、`SemanticId`、`Origin`、semantic attributes。
- `TyVarSymbol`、`TypeSymbol`、`TyVar`、`TyApp`。
- abstract/alias/NoDef 表示；range/float node 可以在 core 表示，但 MVP feature scanner 必须拒绝。
- `core/logic` 只实现 immutable `Ty` 的 substitution、matching、instantiation 和 free vars，不保存 `Fresh/Link`。
- `elab` 拥有 `Dty = Fresh | Link | App | Known`、mutable unification、occurs check 和 strict/non-strict finalize；`Dty/Dterm` 不出现在 semantic public API。
...
- `Dtype`、`Dparam`、非递归 `Dlogic`、`Dprop(Paxiom/Plemma/Pgoal)`。
- core 还要能只读表示 snapshot 中的 `Ddata`（Bool、Tuple0/Unit 等 clone-expanded 声明）。`Ddata` 不由 MVP Ptree elaborator 或普通 public constructor 创建，只能由校验 schema/hash/invariant 的 trusted snapshot decoder 载入；用户 datatype 继续在 elaboration 阶段拒绝。
...
- 独立 `Environment`：同一 context 下的内存 Theory + Pmodule registry，key 为字节路径和 theory/module 名；pure `use` 解析 Theory，module 中的 `use` 解析 Pmodule 及其 pure theory projection。
- 不实现通用 `clone`，但也不手写删减版公理。由固定 Why3 1.7.2 oracle 一次性导出 MVP 所需的 clone-expanded semantic dependency closure与 program export，并生成只读 MoonBit snapshot：
...
- snapshot 的 pure 部分必须逐 item 保留 dependency order、qualified symbol reference、attributes、`Use`、clone source/instantiation witness、driver meta 所引用的 type/logic/prop handle，以及原版完整 algebra declarations/axioms；只保存 clone 结果与历史，不实现 clone evaluator。
...
- manifest 记录导出器版本、源 commit、每个 Theory/Pmodule canonical SHA-256 和总 transform-influence closure SHA-256。
- `minimal_env(context)` 返回 `(Environment, StdlibHandles)`：前者注册两部分 snapshot，后者以稳定 `SnapshotSymbolKey` 索引同一 context 内重新 intern 的 type/logic/prop/routine handle。两者不复用生成时的 runtime ID；内建 operator 绑定 handle，而不是根据源码拼写识别。
...
- Theory/Pmodule snapshot inventory/hash 与 Why3 oracle 导出一致；任意 declaration/routine/meta/rs_logic 缺失立即失败。
...
- full canonical raw Task 与 typed stdlib Pmodule projection（忽略 runtime/context tag）均和同一 OracleContext 的 Why3 oracle 一致。
...
- 支持的 pure declarations：`Dtype/Dlogic/Dprop/Dscope/Duse*`。
- 同一文件中 theory/module 的顺序注册。
- program declaration、clone、datatype 等在此阶段分类为 `UnsupportedFeature`，不能误报成一般 type error。
...
`Z3StaticSpec` 不能持有某次 `minimal_env(context)` 的 `SemanticId`。context-free `SnapshotSymbolKey` 由 `core/identity` 所有，以 theory path、symbol category、qualified export key、declaration ordinal 和 manifest symbol digest 构成，因而 `stdlib` 与 `prover/z3` 无需互相 import。
...
`pipeline` 创建这些回调并捕获同一次 `minimal_env` 返回的 `StdlibHandles`，再调用 `@z3.bind_profile(spec, resolver, context)`；该公开签名不出现 `StdlibHandles`，所以 `prover/z3` 不 import `stdlib`。缺 key、类别/digest 不符或 context 不同立即失败；profile 只能在该 context 生命周期内使用，禁止跨编译全局缓存 typed profile。
...
1. raw 阶段只拒绝用户声明产生的 unsupported；允许带 trusted-origin 标记的 snapshot `Ddata` 和 BuiltIn 多态 equality；
...
5. printer 前再拒绝残余 polymorphism、epsilon 以及非 manifest allowlist/未编码 datatype；trusted monomorphic Tuple0/Unit `Ddata` 是唯一可进入 M6 datatype printer 的例外。
...
- 同一个 `Z3StaticSpec` 可分别绑定两个 context；各自 symbol 均来自对应 `StdlibHandles`，交叉使用返回 `ContextMismatch`。
...
- residual polymorphism、epsilon、用户 datatype 与非 allowlist datatype 一定在 printer 前失败；仅已绑定同 context、hash 校验通过的 Tuple0/Unit snapshot `Ddata` 可继续。
...
- 确定性 identifier allocator；SMT 保留字黑名单；同名冲突处理。
...
- 可额外做 S-expression alpha-normalized diff，仅用于诊断。
...
MoonBit runner 改用 `-in` 是明确记录的 transport divergence：它避免临时文件，但不宣称 argv 与 Why3 profile 字节相同。`CommandSpec` 要按 Why3 `%t` 的整数秒规则生成 `-T:<sec>`；无 solver limit 时省略。oracle 必须同时跑原版 `%f` 命令和 MoonBit stdin 命令，并比较归一化 prover answer；生成的 SMT 字节仍单独精确比较。wall-clock timeout 由父进程强制执行，deadline 应略晚于 Z3 limit 并始终负责最终回收。
...
`Invalid` 先保留在公共结果协议中以避免未来 model/counterexample 阶段破坏 API；当前无 counterexample 的 Z3 profile 不产生该结果。
...
| stdout/stderr 已匹配明确答案 | 先按 Why3 answer pattern 分类；只有父进程强制 timeout/output-limit 才覆盖它 |
| 未匹配答案且非零退出、signal、乱码或无法分类 | `HighFailure` |

通用 `z3_487` profile 没有把普通 stderr/非零退出映射为 `Failure` 的专用规则；`Failure(reason)` 只为未来显式 profile rule 保留。结果 parser 是纯函数，先用 canned stdout/stderr/exit-status 单测，不依赖真实 Z3 制造边界状态，并用 Why3 oracle 锁定“已识别答案优先、进程状态兜底”的顺序。
...
1. `CommandSpec` 保存 executable 和 argv，不保存 shell command。
2. 校验 executable/argument 不含 NUL；不做 shell escaping。
3. 建立 stdin、stdout、stderr 三组 pipe。
4. 在 task group 中 spawn Z3，使用 `hard_cancel()`。
5. 一个任务写完全部 SMT `Bytes` 后立即关闭 stdin。
6. 两个任务并发读取 stdout/stderr，逐块累加。
7. `read_some(max_len=remaining)` 强制每路和总输出上限；默认总计 4 MiB。
8. `with_timeout` 达到 deadline 后 cancel；随后必须 `wait`，不能遗留子进程。
9. 正常、超时、输出超限、写端 broken pipe 和 reader error 的所有路径都关闭 pipe。
10. 返回 `RawProcessOutcome`，再交给 `@z3.parse_result`。
...
```text
why3mbt check file.mlw
why3mbt task file.mlw [-T theory] [-G goal]
why3mbt emit-smt file.mlw [-T theory] [-G goal]
why3mbt prove file.mlw [-P z3] [--z3 PATH] [-t seconds]
```
...
```text
0  所有目标 Valid
1  parse/type/config/unsupported/internal error
2  至少一个目标非 Valid
```
...
- timeout 后子进程已回收。
- solver 不存在、非零退出和 malformed output 分类稳定。
...
- 用户 `Cty` 复用 snapshot routine 的同一表示：args、result、pre、post、effect。
- typed Expr：
...
保留 Kode 层，避免把表达式遍历和公式拼接写成一个以后无法扩展的函数。首版 `Kcont` 只允许 `Kcont 0`；任何其他 continuation depth 都是内部 invariant failure。首版不实现 `Ktag`、`Khavoc`、异常 continuation、SP optimization 和 type invariant injection；在 lowering 前拒绝 `vc:sp`、`vc:wp` 等会选择这些路径的属性。
...
solver result 是最后一层，不是主结构 oracle。

### 7.2 CLI 冒烟命令

所有 Why3 CLI、OCaml oracle、stdlib exporter 和 result oracle 必须读取同一个 `OracleContext` manifest。它至少固定：Why3 executable content hash/version/commit、`LC_ALL=C`、禁用默认插件、`--no-stdlib`、stdlib tree hash、fixture 专属且有序的 repo-relative loadpath + tree hash、driver closure 的相对名 + content hash、prover-detection-data hash、Whyconf 生成 recipe，以及 Z3 version/content hash。

`oracle_context_sha256` 只覆盖这些稳定逻辑身份，不覆盖 checkout/temp root 的绝对路径、临时 `why3.conf` 路径、文件“尚未/已经生成”的状态或 resolved argv。wrapper 每次另外输出不参与 semantic golden 的 `resolved_context.json`，记录绝对 executable/loadpath/datadir/argv、生成后 `whyconf_sha256` 和 driver 实际解析路径。这样同一 manifest 在两个临时绝对根生成的 canonical NDJSON 必须相同，而诊断 artifact 仍可追溯实际进程。
...
两个模式都必须由 7.2 的 `OracleContext` 构造 `Env`：同一个 pinned stdlib 内容、同序 fixture loadpaths、相同 format 注册和已验证 datadir/driver closure；不允许 helper 内部另建默认 `Env.create_env`。每条 NDJSON 都写入稳定的 `oracle_context_sha256`，以证明 typed/raw/prepared/result 来自同一逻辑环境；resolved absolute paths 只进旁路 artifact。
...
```json
{
  ...
  "goal": "abs'vc",
  "expl": ["postcondition"],
  "loc": {"file":"mvp/abs.mlw","bl":4,"bc":4,"el":4,"ec":27},
  "typed_semantic_sha256": "...",
  "why3_raw_task_checksum_v6": "...",
  "raw_task_semantic_sha256": "...",
  "why3_z3_prepared_task_checksum_v6": "...",
  "z3_prepared_semantic_sha256": "...",
  "smt_token_sha256": "...",
  "answer": "Valid"
}
```

Why3 1.7.2 的 `Termcode` shape version 是 6。原生 checksum 适合快速检测 reference 漂移，但不能单独作为 gate，因为它不是完整语义哈希，且不会完整覆盖名称、location、attributes、range/float 参数等信息。MVP 不移植 OCaml `Termcode`：上述两个 `why3_*_checksum_v6` 是 reference-only 诊断字段；MoonBit NDJSON 在同一 schema 位置输出 `null`，比较器不跨实现比较它们。
...
首版不实现通用 `clone`，但 M3 从固定 Why3 1.7.2 导出 `BuiltIn/Bool/Unit/int.Int/real.Real` 的完整 clone-expanded Theory closure 和对应 Pmodule program exports。因此：
...
MoonBit 侧由 all-target `oracle/canonical` 实现唯一 canonicalizer；它是正常 package API，不是 `_test.mbt` 私有 helper。native `cmd/why3 --canonical-json` 调用纯 `pipeline` 取得 typed/raw/prepared 值，再逐 goal 输出与 7.3 同 schema 的 NDJSON。CI 通过该 executable 消费真实 package 边界，禁止 Node 脚本复制一份 canonicalization 逻辑；Node 只负责排序 fixture、校验 schema/hash 和 diff。
...
序列化格式固定为无多余空白的 UTF-8 JSON、schema 字段顺序和 `\n` 行结束；hash 输入是这组精确 bytes。NDJSON 每个 goal 一行，禁止 pretty-printer、平台换行或浮点 formatter 参与 hash。
...
- attributes 按 Bytes 排序，但保留 `stop_split`、`expl:*`、VC/model 属性；
- 整数和有理数用精确字符串；
...
- 用户符号名精确保留；
- 生成符号 alpha rename 只作为辅助诊断，不作为主 gate。
...
PR corpus 建议 30 至 50 个小 fixture，至少覆盖：
...
- raw VC；
- 每个已实现 transform；
- SMT identifier collision、reserved word、goal 取反；
...
5. typed semantic hash。
6. raw Task 原生 checksum 记录 + full canonical semantic hash 强校验。
7. Why3 prepared checksum 记录 + prepared canonical semantic hash 强校验。
8. SMT token exact。
...
- prepared Task/SMT 全量差分；
- 按 goal 分片和资源限额执行 solver；near-limit 结果只做稳定性观测/允许集合，不更新 exact answer golden；
- 保存 Why3/MoonBit raw task、prepared task、SMT、stdout/stderr artifacts。

### 9.3 CI 镜像

当前 Dockerfile 只安装 Why3，没有安装 Z3。加入真实 result gate 前必须：
...
6. 输出到 candidate 目录。
7. 在两个不同临时绝对路径生成两次，canonical NDJSON/golden 必须相同；`resolved_context.json` 可含不同绝对路径，只校验其内容 hash/解析关系，不参与逐字节比较。
8. 展示结构化 diff。
9. 只有显式 `--promote` 才替换 golden。
10. normalizer schema 改变时创建新版本目录，不重解释旧 golden。
...
```text
PR-00  Scope/oracle schema/version locks
PR-01  core/identity
PR-02  core/logic Ty/matching + elab Dty/unification
PR-03  core/logic Symbol + Term + substitution
PR-04  Decl/Known + minimal MLW symbols/Pmodule + Environment/Task + frozen Theory/Pmodule snapshot
PR-05  pure logic elaborator
PR-06  canonical oracle + raw Task differential
PR-07  static Z3 profile + feature scan + transforms
PR-08  SMT-LIB printer + token differential
PR-09  result parser + native runner + pure closure A
PR-10  complete user Expr/Pdecl IR + program elaborator
PR-11  Kode/classical WP + VC differential
PR-12  CLI integration + closure B + CI hardening
```

可以并行：

- PR-00 与 PR-01；
- native runner spike 与 PR-02 至 PR-05；
- SMT token normalizer 与 core development；
- M3 的 MLW symbol/Pmodule 外壳在 Term/Decl 稳定后先落地；用户 Expr/Pdecl 扩展可与 transform/printer 并行。
...
| 完整标准库过早拖入 clone/datatype | 不实现 clone；冻结并校验仅覆盖 BuiltIn/Bool/Unit/int.Int/real.Real 的 clone-expanded Theory + Pmodule closure，trusted `Ddata` 与用户 datatype 分流 |
...
| 逐行翻译的许可义务 | M0 明确许可/clean-room 决策，不把技术计划当法律结论 |
...
8. range/float/bitvector/string；
...
- [ ] Why3 1.7.2、fixture、driver、prover detection、semantic stdlib snapshot、Z3 版本和 hash 已锁定。
...
- [ ] frozen snapshot 保留 trusted Ddata、Use/CloneWitness/Meta history，并与 profile 在同一 context 绑定。
