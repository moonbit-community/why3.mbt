...
function validateTrace(trace, fixture, checkpointSequence) {
  ...
  if (trace.records.length !== fixture.goalCount * stride) {
    fail(
      `${fixture.label}: expected ${fixture.goalCount * stride} reference records, got ${trace.records.length}`,
    );
  }
  const rawLines = [];
...
function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    for (const fixture of FIXTURES) {
      ...
      const reference = parseRecords(
        runTrace(executables.trace, why3Root, snapshot, fixture),
        `Why3 ${fixture.label}`,
        fixture,
        semanticProfile.semanticProfileSha256,
      );
      const expected = validateTrace(reference, fixture, checkpointSequence);
      assertSameOutput(
