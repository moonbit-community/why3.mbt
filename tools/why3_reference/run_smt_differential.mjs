...
function verifyWhy3(options, profile) {
...
function runReference(executable, why3Root, snapshot, fixture) {
  const argv = [
    '--why3-root',
    why3Root,
    '--snapshot',
...
function parseQueryRecords(output, fixture, referenceRecords) {
  if (!output.endsWith('\n')) {
  ...
  return lines.map((line, index) => {
    ...
    if (record.unit_name_hex !== reference.unit_name_hex ||
        record.goal_name_hex !== reference.goal_name_hex ||
        record.goal_ordinal !== reference.goal_ordinal) {
...
function parseOnlyWithZ3(query, label) {
  ...
  if ((result.error !== undefined && result.error.code !== 'EPERM') ||
      result.status !== 0 ||
      result.signal !== null ||
...
function main(argv = process.argv.slice(2)) {
  ...
  try {
    const runtime = verifyWhy3(options, z3Profile);
    ...
    for (const fixture of FIXTURES) {
      const referenceOutput = runReference(
        executables.smtTrace,
        why3Root,
        snapshot,
        fixture,
      );
      const reference = parseCanonicalRecords(
        referenceOutput,
        `Why3 ${fixture.label}`,
        fixture,
      ...
      if (fixture.identifierSafety) validateIdentifierSafety(reference.records[0]);
