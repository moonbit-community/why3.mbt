...
function compileGoalExporter(directory, runtime) {
  ...
  runChecked('ocamlfind', [
    ...
    '-o',
    output,
  ], { cwd: directory, env: referenceRuntimeEnvironment(runtime) });
  return output;
}
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!['--output', '--check', '--why3-root', '--why3-archive'].includes(option)) {
    ...
    if (option === '--output' || option === '--check') {
      ...
      result.mode = option.slice(2);
      result.path = resolve(value);
    } else if (option === '--why3-root') {
      result.why3Root = resolve(value);
      explicitRoot = true;
    } else {
      result.why3Archive = resolve(value);
    }
    index += 1;
  }
...
async function buildManifest(options) {
  ...
  let semanticEntries;
  try {
    const exporter = compileGoalExporter(temporary, runtime);
    ...
    const supported = [
      ['mvp.logic-core', 'tools/why3_reference/fixtures/mvp.mlw', ['LogicCore', 'LogicReal'], ['bool', 'int', 'real', 'uninterpreted-symbols', 'ordinary-attribute'], 'Supported'],
      ['mvp.logic-polymorphism', 'tools/why3_reference/fixtures/mvp.mlw', ['LogicPolymorphism'], ['abstract-type', 'alias-type', 'polymorphism', 'multiple-instantiations'], 'Supported'],
      ['mvp.logic-quantifiers', 'tools/why3_reference/fixtures/mvp.mlw', ['LogicQuantifiers'], ['forall', 'exists', 'trigger'], 'Supported'],
      ['mvp.unit', 'tools/why3_reference/fixtures/mvp.mlw', ['UnitMvp'], ['tuple0', 'unit'], 'Supported'],
      ['mvp.abs', 'tools/why3_reference/fixtures/mvp.mlw', ['Abs'], ['if', 'program-int', 'requires-ensures', 'result', 'raw-vc'], 'Supported'],
      ['mvp.namespace', 'tools/why3_reference/fixtures/mvp.mlw', ['NamespaceMvp'], ['qualified-lookup', 'scope', 'use'], 'Supported'],
      ['mvp.multiple-units', 'tools/why3_reference/fixtures/mvp.mlw', ['MultiFirst', 'MultiSecond'], ['ordered-units', 'goal-inventory'], 'Supported'],
      ['mvp.routine-call', 'tools/why3_reference/fixtures/mvp.mlw', ['RoutineCall'], ['program-call', 'requires-ensures', 'result'], 'Supported'],
      ['mvp.assert-assume', 'tools/why3_reference/fixtures/mvp.mlw', ['AssertAssume'], ['assert', 'assume', 'requires-ensures'], 'Supported'],
      ['mvp.program-real', 'tools/why3_reference/fixtures/mvp.mlw', ['ProgramReal'], ['program-real', 'program-call', 'requires-ensures', 'result'], 'Supported'],
      ...
      ['transform.polymorphism', 'tools/why3_reference/fixtures/transform-polymorphism.mlw', ['LogicPolymorphism'], ['free-type-variable', 'monomorphise-goal', 'polymorphic-checkpoints'], 'Supported'],
      ['transform.polymorphic-definition', 'tools/why3_reference/fixtures/transform-polymorphic-definition.mlw', ['LogicPolymorphicDefinition'], ['polymorphic-definition', 'discriminate', 'encoding-guards'], 'Supported'],
      ['transform.inductive-snapshot', 'tools/why3_reference/fixtures/transform-inductive.mlw', ['InductiveSnapshot'], ['reference-only', 'trusted-inductive', 'eliminate-inductive', 'higher-order-encoding'], 'ReferenceOnlyTrusted'],
      ['smt.identifier-safety', 'tools/why3_reference/fixtures/smt-identifiers.mlw', ['SmtIdentifiers'], ['generated-name-collision', 'goal-negation', 'reserved-smt-identifier'], 'Supported'],
      ['solver.outcomes', 'tools/why3_reference/fixtures/solver-outcomes.mlw', ['SolverOutcomes'], ['solver-sat', 'solver-unknown', 'solver-unsat'], 'Supported'],
    ...
    const unsupportedDefinitions = [
      ...
      id,
      path: `tools/why3_reference/fixtures/unsupported/${file}`,
      unitNames,
...
async function main() {
  ...
  const rendered = `${JSON.stringify(await buildManifest(arguments_), null, 2)}\n`;
