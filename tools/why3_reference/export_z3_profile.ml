...
(*                                                                  *)
(*  MoonBit Z3-profile reference adapter, modified 2026-07-21.         *)
(*                                                                  *)
(********************************************************************)
...
let () =
  ...
  let driver =
    Driver.load_driver_file_and_extras main environment ~extra_dir:None
      !root_driver []
  in
  let profile = Driver.reference_profile_view driver in
  let meta_groups =
    List.map
      ...
      profile.Driver.reference_metas
  in
  let prelude_groups =
    List.map
      (fun (ident, values) ->
      ...
      profile.Driver.reference_theory_preludes
  in
  let use_groups =
    List.map
      (fun (source, export) ->
        ...
        `Assoc
          [ ("theoryKey", json_string (theory_key source));
            ("uses", `List uses) ])
      profile.Driver.reference_theory_uses
  in
  let result_regexps =
    List.map
      (fun (pattern, answer) ->
      ...
      profile.Driver.reference_result_parser.Call_provers.prp_regexps
  in
  let result_exit_codes =
    List.map
      (fun (code, answer) ->
        `Assoc [ ("code", json_int code); ("answer", answer_json answer) ])
      profile.Driver.reference_result_parser.Call_provers.prp_exitcodes
  in
  let output =
    `Assoc
      [ ("schemaVersion", json_int 1);
        ...
        ("printer", json_string (required "printer" profile.Driver.reference_printer));
        ("filename", json_string (required "filename" profile.Driver.reference_filename));
        ("transforms", `List (List.map json_string profile.Driver.reference_transforms));
        ("preludes", `List (List.map json_string profile.Driver.reference_preludes));
        ...
        ("blacklist", `List (List.map json_string profile.Driver.reference_blacklist)) ]
  in
