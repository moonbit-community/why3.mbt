...
export function renderCanonicalVectors(kind) {
  ...
  if (kind === 'stage05') {
    return `${JSON.stringify({
      ...
      stage: 5,
      hashInputIncludesFinalLf: true,
      vectors: vectors(stage05Cases),
    }, null, 2)}\n`;
  }
  fail(`unknown canonical vector set ${kind}`);
}

export function renderStage05CanonicalVectors() {
  return renderCanonicalVectors('stage05');
}

function main(argv) {
  const kind = argv[0];
  const output = argv.slice(1);
  if (!['schema', 'stage05'].includes(kind) ||
      (output.length !== 0 &&
       (output.length !== 2 || !['--output', '--check'].includes(output[0])))) {
    fail(
      'usage: generate_canonical_vectors.mjs (schema|stage05) ' +
