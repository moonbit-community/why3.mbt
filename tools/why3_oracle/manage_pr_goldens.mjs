...
const GOLDEN_ROOT = join(SCRIPT_DIRECTORY, 'goldens', 'pr-v1');
...
const FIXTURES = [
  { label: 'tools/why3_oracle/fixtures/mvp.mlw', polymorphic: false },
...
function parseArguments(argv) {
  if (argv.length === 0 || (argv.length === 1 && argv[0] === '--check')) {
    return { mode: 'check', directory: GOLDEN_ROOT };
  }
  ...
  fail(
    'usage: manage_pr_goldens.mjs [--check | --candidate DIR | --promote DIR]',
  );
}
...
function buildArtifacts() {
  ...
  const temporary = mkdtempSync(join(tmpdir(), 'why3mbt-pr-golden-'));
  try {
    ...
    const manifestBase = {
      schemaVersion: 1,
      profile: 'why3-1.7.2-pr-golden-v1',
    ...
    const manifest = {
      ...manifestBase,
      goldenSetSha256: canonicalSha(manifestBase),
    };
...
function writeCandidate(directory, files) {
  ...
  for (const [name, content] of files) {
    const current = join(GOLDEN_ROOT, name);
...
function promoteCandidate(directory) {
  ...
  for (const [name, content] of candidate) {
    writeFileSync(join(GOLDEN_ROOT, name), content);
  }
  process.stdout.write(
    `manage_pr_goldens: promoted ${candidate.size} files from ${directory}\n`,
  );
}
...
function main() {
  ...
  if (arguments_.mode === 'candidate') {
    ...
    process.stdout.write(
      `manage_pr_goldens: ${generated.size} checked-in files exact across two roots\n`,
    );
  }
}
...
try {
  ...
  process.stderr.write(`manage_pr_goldens: ${error.message}\n`);
  process.exitCode = 1;
}
