...
function verifyWhy3SourceAndTools(driver, options, buildDirectory) {
  let root;
...
function oracleArguments(why3Root, snapshot, fixture, file) {
  const argv = [
    '--stdlib',
...
function runOracle(executable, why3Root, snapshot, fixture, file) {
  return runChecked(
    executable,
    oracleArguments(why3Root, snapshot, fixture, file),
    {
      cwd: PROJECT_ROOT,
      env: { ...process.env, LC_ALL: 'C' },
      maxBuffer: 128 * 1024 * 1024,
...
function validatePortability(
  ...
  for (const name of ['absolute-root-a', 'absolute-root-b']) {
    ...
    outputs.push({
      typed: runOracle(executables.typed, why3Root, snapshot, fixture, file),
      raw: runOracle(executables.raw, why3Root, snapshot, fixture, file),
    });
  }
  assertSameOutput(
    primaryTyped,
...
function main(argv = process.argv.slice(2)) {
  ...
  try {
    const why3Root = verifyWhy3SourceAndTools(
      driver,
      options,
      buildDirectory,
    );
    ...
    if (process.env.WHY3_ORACLE_KEEP_TMP === '1') {
