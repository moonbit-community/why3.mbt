...
const MVP_FIXTURE = 'tools/why3_oracle/fixtures/mvp.mlw';
const POLYMORPHISM_FIXTURE =
  'tools/why3_oracle/fixtures/transform-polymorphism.mlw';
const POLYMORPHIC_DEFINITION_FIXTURE =
  'tools/why3_oracle/fixtures/transform-polymorphic-definition.mlw';
const IDENTIFIER_FIXTURE = 'tools/why3_oracle/fixtures/smt-identifiers.mlw';
const SOLVER_FIXTURE = 'tools/why3_oracle/fixtures/solver-outcomes.mlw';
...
function parseArguments(argv) {
  if (argv.length === 0 || (argv.length === 1 && argv[0] === '--check')) {
    return { mode: 'check', path: RESULT_GOLDEN };
  }
...
function assertDetectedZ3Profile(configText) {
  const expectedCommand =
    `command = "'${LOCK.z3.executable.path}' -smt2 -T:%t ` +
...
function main() {
  ...
  if (arguments_.mode !== 'check' && arguments_.path === RESULT_GOLDEN) {
    fail('candidate path must be separate from the checked-in result golden');
  }
  const corpusBytes = readFileSync(CORPUS_PATH);
  const corpus = JSON.parse(corpusBytes);
  const cases = bindCasesToCorpus(corpus);
  if (process.env.WHY3_ORACLE_IMAGE_DIGEST !== LOCK.image.digest) {
    fail(
      `WHY3_ORACLE_IMAGE_DIGEST must equal locked ${LOCK.image.digest}; ` +
      'run this gate inside the fixed oracle image',
    );
  }
  buildCli();
  ...
  try {
    ...
    const resultBase = {
      ...
      oracleImageDigest: LOCK.image.digest,
      z3: {
        version: LOCK.z3.version,
        executableSha256: LOCK.z3.executable.sha256,
      },
      compared,
    };
    const result = {
    ...
    if (arguments_.mode === 'candidate') {
      ...
      const before = existsSync(RESULT_GOLDEN)
        ? sha256(readFileSync(RESULT_GOLDEN))
        : null;
      ...
      mkdirSync(dirname(RESULT_GOLDEN), { recursive: true });
      writeFileSync(RESULT_GOLDEN, rendered);
      process.stdout.write(
        `run_result_differential: promoted ${compared.length} result records\n`,
      );
    } else {
      if (!existsSync(RESULT_GOLDEN) ||
          readFileSync(RESULT_GOLDEN, 'utf8') !== rendered) {
        fail('checked-in prover-result golden differs from fixed-image results');
      }
      process.stdout.write(
